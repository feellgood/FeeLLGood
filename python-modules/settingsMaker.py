...
## \class Settings
# this class gives an efficient way to build and write a json file from a dictionary to be able to
# script easily feellgood executable
...
class Settings(object):
    ...
    def __init__(self,mshFileName):
        ## \brief dictionary
        # mySets is the strict equivalent to the json file needed by executable feellgood to run a
        # simulation, any (key,value) not corresponding to what is expected by feellgood is just
        # ignored.
        self.mySets = {
            "outputs": {},
            "mesh": {
    ...
    def createSTT(self,volRegRef,sigma,N0,beta,l_J,l_sf,bc_surf_reg_ref1, bc_val1, bc_surf_reg_ref2,
            bc_val2):
        self.mySets["spin_transfer_torque"] = {
            "enable": True,
            "volume_region_reference" : str(volRegRef),
            "sigma" : sigma,
            "dens_state" : N0,
            "beta" : beta,
            "l_J" : l_J,
            "l_sf" : l_sf,
            "boundary_conditions" : {
                { str(bc_surf_reg_ref1) : bc_val1 },
                { str(bc_surf_reg_ref2) : bc_val2 }
            }
        }
    ...
    # this method is called before writing the json file, it might also be called from another
    # script for some checking before calling the write method
    def check(self):
