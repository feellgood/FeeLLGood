...
for (int t=0; t<TET; t++){
    ...
    tet.integrales(settings.paramTetra,fem.node,fem.Hext,fem.DW_vz,settings.theta,dt,settings.TAUR,K, L);     
...
if (!nt) {
    if(VERBOSE) { cout << "%5t computing prc %30T.";fflush(NULL); }
...
if (!(bicg_iter.converged())) {
    time(&timeEnd);
	if(VERBOSE) //cout << boost::format("%5t  bicg FAILED in %d %30T. ") % bicg_iter.get_iteration() 
	...
	time(&timeEnd);
    if(VERBOSE) { cout << "%5t gmres FAILED in "//boost::format("%5t gmres FAILED in ")
         << gmr_iter.get_iteration() << " !! ......... " << difftime(timeEnd,timeStart) << " s" << endl; }
    return 1;
    }
    else {time(&timeEnd);
        if(VERBOSE) { cout << "%5t v-solve in " //boost::format("%5t v-solve in ") 
