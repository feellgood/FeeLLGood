...
void projection_vect(T &x, Pt::pt3D *B)
    ...
    for (int i = 0; i < (2 * N); i++)
        ...
        for (int k = 0; k < N; k++)
            {
            x.Lp[i] += Nodes::Pcoeff<T>(x, i, k) * B[k].x();
            }
        for (int k = 0; k < N; k++)
            {
            x.Lp[i] += Nodes::Pcoeff<T>(x, i, N + k) * B[k].y();
            }
        for (int k = 0; k < N; k++)
            {
            x.Lp[i] += Nodes::Pcoeff<T>(x, i, 2 * N + k) * B[k].z();
            }
        }
    }
...
void projection_mat(T &x, double (&A)[3 * N][3 * N])
    ...
    for (int i = 0; i < (2 * N); i++)
        ...
        for (int k = 0; k < (3 * N); k++)
            ...
            for (int j = 0; j < (3 * N); j++)
                {
                PA[i][k] += Nodes::Pcoeff<T>(x, i, j) * A[j][k];
                }
            }
        }
    ...
    for (int i = 0; i < (2 * N); i++)
        for (int k = 0; k < (2 * N); k++)
            ...
            for (int j = 0; j < (3 * N); j++)
                {
                x.Kp[i][k] += PA[i][j] * Nodes::Pcoeff<T>(x, k, j);
                }
            }
    }
