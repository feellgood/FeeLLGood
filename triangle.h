...
class Triangle {
	public:
        ...
        if (_NOD>0)
            ...
            if((i0>0)&&(i0<=_NOD)) { ind[0]--; } else std::cout<< "warning index i0 out of bounds in Triangle constructor" <<std::endl;
            if((i1>0)&&(i1<=_NOD)) { ind[1]--; } else std::cout<< "warning index i1 out of bounds in Triangle constructor" <<std::endl;
            if((i2>0)&&(i2<=_NOD)) { ind[2]--; } else std::cout<< "warning index i2 out of bounds in Triangle constructor" <<std::endl;
            }
        }
