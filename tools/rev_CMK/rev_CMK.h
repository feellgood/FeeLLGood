...
inline T str2num(const std::string &s)
    ...
    T x;
    if (!(iss >> x)) throw BadConversion("convertToDouble(\"" + s + "\")");
    return x;
    }
...
void update_labelling(std::string inputFileName, std::vector<int> &old2newlabel,
    ...
    for (int nod = 1; nod <= NOD; nod++)
        ...
        fout << boost::format("%d %.18g %.18g %.18g") % nod % coord[i].x % coord[i].y % coord[i].z
             << std::endl;
        }
